...
<%
    ...
    if ("true".equals(request.getParameter("update"))) {
        ...
        ps = con.prepareStatement(
            "UPDATE ngo SET name=?, email=?, contact=?, location=? WHERE ngo_id=?"
        );
        ps.setString(1, name);
        ...
        out.println("<script>alert('NGO updated successfully!'); window.location='admin_users.jsp';</script>");
        return;
    }

    // ---------- FETCH RECORD FOR EDIT ----------
    String idParam = request.getParameter("id");
    ResultSet ngoData = null;

    if (idParam != null) {
        ps = con.prepareStatement("SELECT * FROM ngo WHERE ngo_id=?");
        ps.setInt(1, Integer.parseInt(idParam));
        ngoData = ps.executeQuery();
    }
%>
...
<div class="edit-card">
<%
    if (ngoData != null && ngoData.next()) {
%>
<h2>Edit NGO</h2>
<form action="editNgo.jsp" method="post">
    <input type="hidden" name="update" value="true">
    <input type="hidden" name="id" value="<%= ngoData.getInt("ngo_id") %>">

    <label>Name:</label>
    <input type="text" name="name" value="<%= ngoData.getString("name") %>" required>

    <label>Email:</label>
    <input type="email" name="email" value="<%= ngoData.getString("email") %>" required>

    <label>Contact:</label>
    <input type="text" name="contact" value="<%= ngoData.getString("contact") %>">

    <label>Location:</label>
    <input type="text" name="location" value="<%= ngoData.getString("location") %>">
